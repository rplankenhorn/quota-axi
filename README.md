...
`apiEquivalentUsd` reprices the recorded tokens at **standard global API reference rates**, including cache categories and verified per-request long-context thresholds. It is not a historical tariff lookup and does not include fast/priority or regional premiums, batch discounts, tool fees, taxes, or subscription charges. Pi's stored `usage.cost` is not trusted as billing evidence. Unverified models/rates, unfamiliar token counters, or Claude cache writes without an established retention split leave the record unpriced; its native tokens remain visible. Older Claude long-context requests without a verified tariff likewise stay unpriced. A Pi `openai-codex` record whose `cacheReadTokens` do not exceed its `inputTokens` cannot be distinguished from OpenAI's native cache-inclusive `input` shape, so rather than risk overstating fresh input it is left unpriced (`pi_codex_input_units_unverified`) with its native tokens preserved; a record whose cache reads exceed input proves the cache-exclusive shape and is priced.
